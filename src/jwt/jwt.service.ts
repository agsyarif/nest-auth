...
import { v4 } from 'uuid';
import { IUserAccessPayload, IUserAccessToken } from './interfaces/user-access-token.interface';
...
export class JwtService {
  ...
  private static async generateTokenAsync(
    payload: IEmailPayload | IRefreshPayload | IUserAccessPayload,
    secret: string,
    ...
    switch (tokenType) {
      case TokenTypeEnum.ACCESS:
        ...
        return this.commonService.throwInternalError(
          JwtService.generateTokenAsync({ id: user.id,  user: user }, privateKey, {
            ...jwtOptions,
  ...
  private static async throwBadRequest<
    T extends IUserAccessToken | IRefreshToken | IEmailToken,
